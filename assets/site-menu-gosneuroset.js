...
function squareMenu(){
 ...
 section.innerHTML=`<div class="wrap"><div class="ptg-square-menu-head"><div><div class="ptg-menu-kicker">НАВИГАЦИЯ PROMTAGRAM</div><h2>Весь сайт.<br>На одном экране.</h2></div><p>Выберите раздел. Квадратная карта связывает коммерческий, технологический и социальный контуры Promtagram.</p></div><nav class="ptg-square-grid" aria-label="Основные разделы сайта">${menuItems.map(x=>`<a class="ptg-menu-tile" href="${x.href}"><span class="ptg-menu-num">${x.n}</span><span class="ptg-page-icon"><span class="ptg-page-face">${x.icon}</span></span><span class="ptg-menu-copy"><strong>${x.title}</strong><small>${x.meta}</small></span><span class="ptg-menu-arrow" aria-hidden="true">↗</span></a>`).join('')}</nav></div>`;
 hero.insertAdjacentElement('afterend',section);
}
function moveGosneuroset(){
 const mission=document.querySelector('.reference-home #mission');
 const section=document.querySelector('.reference-home #recognition')||document.querySelector('.reference-home #gosneuroset');
 if(!mission||!section)return;
 section.id='gosneuroset';section.classList.add('ptg-gosneuroset-moved');
 if(mission.previousElementSibling!==section)mission.parentNode.insertBefore(section,mission);
}
...
function syncMenuLinks(){
 ...
 if(![...nav.querySelectorAll('a')].some(a=>a.getAttribute('href')==='/#gosneuroset'||a.getAttribute('href')==='#gosneuroset')){
   const mission=[...nav.querySelectorAll('a')].find(a=>a.getAttribute('href')==='/#mission'||a.getAttribute('href')==='#mission');
   const a=document.createElement('a');a.href='/#gosneuroset';a.textContent='Госнейросеть';a.addEventListener('click',()=>closeNav(nav));
   mission?mission.insertAdjacentElement('beforebegin',a):nav.appendChild(a);
 }
}
function apply(){if(!document.body.classList.contains('reference-home'))return;squareMenu();moveGosneuroset();syncMenuLinks();}
if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',apply,{once:true});else apply();
window.addEventListener('load',()=>{apply();setTimeout(apply,180);setTimeout(apply,800);setTimeout(apply,1600);},{once:true});
})();