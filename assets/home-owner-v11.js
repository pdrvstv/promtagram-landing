(()=>{
  ...
  const BUILD='20260917-owner-v11';
  const pages=[
  ...
  function assignAnchors(){
    [['.ptg-stats','stats'],['.ref-structure','financial-structure'],['.ref-process','process']].forEach(([sel,id])=>{
      const el=document.querySelector(sel);if(el&&!el.id)el.id=id;
    });
  }
  ...
  function partnershipScreen(){
    ...
    if(!sec)return;
    sec.className='section ptg-market-proof ptg-partnership-v11';
    sec.innerHTML=`<div class="wrap">
      ...
      <div class="ptg-market-proof-head">
        ...
        <p>Два независимых внешних подтверждения: публичное присутствие на карте игроков российского ИИ-рынка и профиль Promtagram, сформированный командой ТехноБрокера.</p>
      </div>
      <div class="ptg-market-proof-grid">
        <a class="ptg-market-card ptg-neirolab-card" href="https://neirolab-ai.ru/list" target="_blank" rel="noopener noreferrer">
          <div class="ptg-market-card-meta"><span>ЦТИ «НЕЙРОЛАБ» / КАРТА ИГРОКОВ ИИ</span><span>ПРОВЕРИТЬ ↗</span></div>
          <div class="ptg-market-mark">N/AI</div>
          <h3>Promtagram на карте игроков российского рынка ИИ</h3>
          <p>Promtagram включён в публичную карту ЦТИ «Нейролаб». Внешняя ссылка ведёт непосредственно на список игроков рынка для независимой проверки.</p>
          <div class="ptg-market-proofline"><span>ИИ-рынок</span><span>Публичная карта</span><span>Независимая проверка</span></div>
          <div class="ptg-market-cta">Открыть карту Нейролаб ↗</div>
        </a>
        <article class="ptg-market-card ptg-techbroker-card">
          ...
          <div class="ptg-mail-window">
            ...
            <div class="ptg-mail-body">
              ...
              <div class="ptg-mail-row"><span>Материал</span><b>Профиль Promtagram AI/GR Project Office</b></div>
              <p class="ptg-mail-quote">По итогам анализа команда сформировала профиль Promtagram и отдельно описала внутренний пилот как действующий операционный контур.</p>
              <small class="ptg-mail-note">Публично показываем проверяемый смысл письма без служебной переписки. Это аналитическое профилирование, а не сертификат или государственная аккредитация.</small>
            </div>
          </div>
          <h3>Профиль Promtagram для «Кейсбука ИИ-решений»</h3>
          <p>ТехноБрокер направил Promtagram подготовленный профиль для ознакомления и проверки фактических неточностей. Это внешняя фиксация нашего рабочего ИИ-контура отраслевой командой.</p>
  ...
  function renumber(){pages.forEach(p=>setLabel(document.querySelector(p.sel),p.n,p.title));}
  ...
  function fullSiteMap(){
    ...
    if(head){
      ...
      if(desc)desc.textContent='Все экраны главной страницы в фактическом порядке — от цифр и AI-системы до партнёрства, публикаций, доказательств и контакта.';
    }
    const nav=map.querySelector('.ptg-square-grid');
    if(!nav)return;
    nav.innerHTML=pages.map((p,i)=>`<a class="ptg-menu-tile ptg-menu-tile-v11" href="${p.href}"><span class="ptg-menu-num">${p.n}</span><span class="ptg-page-icon"><span class="ptg-page-face">${icon(i)}</span></span><span class="ptg-menu-copy"><strong>${esc(p.title)}</strong><small>${esc(p.meta)}</small></span><span class="ptg-menu-arrow" aria-hidden="true">↗</span></a>`).join('');
  }
  ...
  function verifyOrder(){
    ...
    const photobank=document.querySelector('#photobank');
    const ok=media&&evidence&&main.indexOf(evidence)===main.indexOf(media)+1&&partner&&photobank&&main.indexOf(partner)===main.indexOf(photobank)-1;
    document.documentElement.dataset.ptgOwnerV11=ok?'ready':'check';
  }
